...
public class NhanVienController {
    ...
    public String chiTiet(@PathVariable("id") Integer id, Model model, RedirectAttributes redirectAttributes){
        NhanVien nv= nhan_vienrepository.findById(id).get();
        redirectAttributes.addFlashAttribute("message1", nv.getTen());
        redirectAttributes.addFlashAttribute("message2", nv.getMaNV());
        redirectAttributes.addFlashAttribute("message3", nv.getTenDangNhap());
        redirectAttributes.addFlashAttribute("message4", nv.getVaiTro().getTenVaiTro());
        redirectAttributes.addFlashAttribute("message5", nv.getTrangThai());

        return "redirect:/nhan-vien";
    }

    @GetMapping("detail/exit")
    public String exit(){
        return "redirect:/nhan-vien";
    }
